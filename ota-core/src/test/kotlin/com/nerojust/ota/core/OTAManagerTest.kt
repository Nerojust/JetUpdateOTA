...
class OTAManagerTest {
    ...
    @Test
    fun `checkForUpdate delegates to the checker`() =
        runTest {
    ...
    @Test
    fun `checkForUpdate records the pending version when an update is available`() =
        runTest {
            val checker = mockk<UpdateChecker>()
    ...
    @Test
    fun `checkForUpdate preserves the existing pending version when up to date`() =
        runTest {
            val checker = mockk<UpdateChecker>()
    ...
    @Test
    fun `downloadUpdate delegates to the downloader and records the downloaded path on completion`() =
        runTest {
    ...
    @Test
    fun `installUpdate does not clear state on success`() =
        runTest {
    ...
    @Test
    fun `installUpdate does not clear state on failure`() =
        runTest {
            val installer = mockk<UpdateInstaller>()
            val apkFile = File("/tmp/update_2.apk")
    ...
    @Test
    fun `currentState delegates to the state store`() =
        runTest {
            val stateStore = mockk<UpdateStateStore>()
