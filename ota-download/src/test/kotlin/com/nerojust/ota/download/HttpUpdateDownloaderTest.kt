...
class HttpUpdateDownloaderTest {
    ...
    @Test
    fun `downloads the full file and emits Complete with a matching checksum`() = runTest {
    ...
    @Test
    fun `restarts from scratch when the server ignores the Range header and returns 200`() = runTest {
        val fullContent = "HELLOWORLD"
        val partial = "HELLO"
        val targetFile = File(targetDir, "update_2.apk")
        targetFile.writeText(partial)
    ...
    @Test
    fun `emits Failed and deletes the file when the checksum does not match`() = runTest {
    ...
    @Test
    fun `emits Failed when the server responds with an error status`() = runTest {
