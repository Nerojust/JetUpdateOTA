...
class HttpUpdateCheckerTest {
    ...
    @Test
    fun `returns Available when server reports an update`() =
        runTest {
            server.enqueue(
    ...
    @Test
    fun `returns UpToDate when server reports no update with no reason`() =
        runTest {
    ...
    @Test
    fun `returns UpToDate when server reports reason up_to_date`() =
        runTest {
    ...
    @Test
    fun `returns NotInRollout when server reports reason not_in_rollout`() =
        runTest {
    ...
    @Test
    fun `sends app_version and device_id as query parameters`() =
        runTest {
            server.enqueue(MockResponse().setBody("""{"has_update":false}"""))
    ...
    @Test
    fun `returns Failed when server responds with an error status`() =
        runTest {
    ...
    @Test
    fun `returns Failed when the connection fails`() =
        runTest {
            server.shutdown()
    ...
    @Test
    fun `returns Failed when the response body is malformed JSON`() =
        runTest {
